...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Madar") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "Madar", DisplayName = "Madar API", Resources = { "Madar" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["Madar_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["Madar_Swagger:RootUrl"]?.TrimEnd('/');
